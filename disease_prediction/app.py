...
# --- CACHE & LOAD DATA AND MODEL ---
@st.cache_resource
def load_and_train_model():
    # 1. Load data
    data = pd.read_csv('improved_disease_dataset.csv')
    
    # 2. Encode disease target
    ...
    # 3. Handle categorical column if present (e.g., gender) and missing values
    if 'gender' in X.columns:
        le = LabelEncoder()
        X['gender'] = le.fit_transform(X['gender'])
    X = X.fillna(0)
    
    # 4. Resampling (using RandomOverSampler from your notebook)
    ...
    return rf_model, encoder, X

# Load components
try:
    rf_model, encoder, X_columns = load_and_train_model()
    data_loaded = True
...
st.write("Enter or select your symptoms below to get a predicted diagnosis using a Random Forest classifier.")

if data_loaded:
    symptoms_list = list(X_columns.columns)
    ...
    if st.button("Predict Disease", type="primary"):
        if not selected_symptoms:
            ...
            rf_pred = encoder.classes_[pred_encoded]

            # Display Result
            st.success("Prediction Complete!")
            st.markdown(f"### Predicted Disease: **{rf_pred}**")
            
            with st.expander("View Input Feature Vector"):
                st.write(input_df)
else:
    st.info("Please ensure that `improved_disease_dataset.csv` is in the same directory as this script.")
=======
import joblib
import os
from sklearn.ensemble import RandomForestClassifier
...
>>>>>>> 6ae4c34ceeaa6a632d528e97430e5cf85815b79d
